...
module DeepiriVizult
  class CLI < Thor
    ...
    option :siblings_scan, type: :boolean, default: false, desc: "Run full scan per sibling repo and merge (sb* prefixed ids)"
    option :infer_org_links, type: :boolean, default: false, desc: "Add low-confidence same_org edges between sibling pairs sharing git org"
    option :org, type: :string, desc: "GitHub org: add remote repo nodes via gh CLI"
    option :submodules, type: :boolean, default: false, desc: "Also scan each .gitmodules checkout and merge with prefixed node ids"
    ...
    def diff(ref = "HEAD")
      ...
      unless $?.success? && !old_raw.strip.empty?
    ...
    def write_outputs(graph, out_dir, format)
      ...
      fmt = format.to_s
      if fmt == "all" || fmt == "json"
        Renderers::JsonRenderer.new(graph).write(File.join(out_dir, "graph.json"))
      end
      if fmt == "all" || fmt == "mermaid"
        Renderers::MermaidRenderer.new(graph).write_all(out_dir)
      end
      if fmt == "all" || fmt == "html"
        Renderers::HtmlRenderer.new(graph).write(File.join(out_dir, "index.html"))
      end
      if fmt == "all" || fmt == "dot"
        Renderers::DotRenderer.new(graph).write_all(out_dir)
      end
      if fmt == "all" || fmt == "csv"
